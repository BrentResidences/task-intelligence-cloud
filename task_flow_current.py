...
# Version 18: compact true wrapped navigation tabs; removes oversized Streamlit button-card tabs; keeps V14 reconnect fixes.
...
def open_new_pg_connection():
    """Open a fresh Neon/Postgres connection with lightweight keepalive settings."""
...
def get_session_pg_connection():
    """Reuse one Neon/Postgres connection per Streamlit session.

    Neon can close an idle cloud connection. The earlier cloud version reused
    the same connection for speed, but if Neon closed it between page actions,
    Streamlit would intermittently raise psycopg2.InterfaceError. This function
    now pings the saved connection before using it and silently reconnects if it
    is stale.
    """
    if not DATABASE_URL:
        raise RuntimeError("DATABASE_URL is missing. Add it in Streamlit Secrets or the local .env file.")

    raw = st.session_state.get("_task_flow_pg_conn")
    try:
        closed = raw is None or raw.closed != 0
    except Exception:
        closed = True

    if not closed:
        try:
            with raw.cursor() as ping_cur:
                ping_cur.execute("SELECT 1")
                ping_cur.fetchone()
        except Exception:
            reset_session_pg_connection()
            raw = None
            closed = True

    if closed:
        raw = open_new_pg_connection()
        st.session_state["_task_flow_pg_conn"] = raw
...
def reset_session_pg_connection():
    raw = st.session_state.get("_task_flow_pg_conn")
    try:
        if raw is not None and raw.closed == 0:
            raw.close()
    except Exception:
        pass
    st.session_state.pop("_task_flow_pg_conn", None)
...
PRIORITY_TYPES = ["Today", "Soon", "As Needed", "Scan / File"]
...
class CloudConnection:
    def __init__(self):
        self.raw = get_session_pg_connection()
    ...
    def close(self):
        # Session connection is intentionally kept open for faster cloud saves.
        return None
    ...
    def __exit__(self, exc_type, exc, tb):
        try:
            ...
            # If Neon closed the session while the page was idle, reset the
            # cached connection. Do not let a rollback/commit failure mask the
            # next page load; the next rerun will reconnect cleanly.
            reset_session_pg_connection()
        # Do not close; reuse this connection during the session for speed.
        return False
...
def conn():
    # Cloud/iPhone version: reads and writes Neon/Postgres using DATABASE_URL.
...
def sync_priority_and_work_type_values():
    ...
    with conn() as c:
        ...
        cur.execute("UPDATE items SET priority_type='As Needed' WHERE priority_type IS NULL OR TRIM(priority_type)='' OR priority_type IN ('Normal', 'Backlog')")
...
def delete_priority_type(name, replacement="As Needed"):
    name = (name or "").strip()
    replacement = (replacement or "").strip() or "As Needed"
    if not name:
        return False, "Priority name is required."
    if replacement == name:
...
def build_assignee_report_df(assignee_name="All", priority_filter="All", subject_filter="All", include_completed=False):
    ...
    if priority_filter != "All":
        df = df[df["priority_type"].fillna("As Needed") == priority_filter].copy()
    ...
    df["Priority"] = df["priority_type"].fillna("As Needed")
    ...
    with conn() as c:
        ...
        for row in rows:
            ...
            if row["priority_type"] is None or str(row["priority_type"]).strip() == "":
                c.execute("UPDATE items SET priority_type='As Needed' WHERE id=?", (row["id"],))
            try:
...
# Schema/setup is expensive against a cloud database, so run it once per session
# instead of on every Streamlit rerun/button click.
if not st.session_state.get("_task_flow_schema_ready", False):
    ensure_schema()
    ensure_subjects_table()
    ensure_work_types_table()
    ensure_priority_types_table()
    ensure_assignees_table()
    seed_subjects_if_empty()
    seed_work_types_if_empty()
    seed_priority_types_if_empty()
    sync_priority_and_work_type_values()
    backfill()

    # Rename legacy Project header values to Ideas while keeping the existing project_id database structure intact.
    with conn() as c:
        c.execute("UPDATE items SET item_type='Ideas' WHERE item_type='Project'")
        c.execute("UPDATE items SET work_type='Ideas' WHERE work_type='Project'")
        c.commit()
    invalidate_caches()
    st.session_state["_task_flow_schema_ready"] = True


def add_item(title, item_type="Task", work_type="Unassigned", subject="Unassigned", priority_type="As Needed", notes="", project_id=None,
    ...
    final_priority_type = priority_type if priority_type in available_priority_types else "As Needed"
...
def remove_item(item_id):
    with conn() as c:
        c.execute("UPDATE items SET in_today=0, priority_type='As Needed' WHERE id=?", (item_id,))
    invalidate_item_caches()
...
def generate(n=5, include_project_filler=True):
    ...
    today = date.today()
    actionable = df[(df["item_type"].isin(["Task", "Reminder", "Appointment"])) & (df["priority_type"].fillna("As Needed") != "Scan / File")].copy()
    forced = []
    forced_ids = set()
    ...
    if len(selected) < n:
        backlog_df = df[df["priority_type"].fillna("As Needed") == "Scan / File"].copy()
...
def load_today():
    return pd.read_sql("""
        ...
        FROM items
        WHERE (
              LOWER(TRIM(COALESCE(priority_type, 'As Needed'))) = 'today'
...
def add_scored_today(max_items=5):
    ...
    today = date.today()
    actionable = df[(df["item_type"].isin(["Task", "Reminder", "Appointment", "Ideas", "Project"])) & (df["priority_type"].fillna("As Needed") != "Scan / File")].copy()
    if actionable.empty:
        return 0
...
def weekly_bucketed_rows():
    ...
    for _, row in weekly_df.iterrows():
        ...
        if row.get("priority_type", "As Needed") == "Scan / File":
...
def render_editable_item_results(df_items, prefix):
    ...
    for r in df_items.itertuples():
        with st.container(border=True):
            ...
            priority_text = r.priority_type if pd.notna(r.priority_type) and str(r.priority_type).strip() else "As Needed"
            ...
            with ec4:
                ...
                current_priority = r.priority_type if r.priority_type in priority_choices else "As Needed"
...
def render_fast_reschedule_results(df_items, prefix="fast_reschedule"):
    ...
    for r in df_items.itertuples():
        with st.container(border=True):
            ...
            if getattr(r, "priority_type", "As Needed") == "Scan / File":
...
def build_fast_reschedule_editor_df(df_items, proj_map):
    ...
    for _, row in df_items.iterrows():
        ...
        rows.append({
            "ID": int(row.get("id")),
            "Title": str(row.get("title", "") or ""),
            "Priority": str(row.get("priority_type", "") or "As Needed"),
...
def apply_fast_reschedule_filters(
    ...
    if priority_type != "All":
        df = df[df["priority_type"].fillna("As Needed") == priority_type]
    if project_name != "All":
...
def build_items_report_pdf(report_df, report_title="Items Report", filter_summary=""):
    buffer = BytesIO()
    doc = SimpleDocTemplate(
        buffer,
        pagesize=landscape(letter),
    ...
    def build_report_table(df_part):
        table_data = [[
            ...
            Paragraph("<b>Title</b>", header_style),
            Paragraph("<b>Work Type</b>", header_style),
            Paragraph("<b>Today Order</b>", header_style),
            ...
            for _, row in df_part.iterrows():
                ...
                if bool_from_db(row.get("is_appointment", 0)):
                    ...
                    if format_date_safe(row.get("appointment_date", "")):
                        appt_text += f" - {format_date_safe(row.get('appointment_date', ''))}"
                reminder_text = ""
                if bool_from_db(row.get("is_reminder", 0)):
                    reminder_text = "Yes"
                    if format_date_safe(row.get("reminder_date", "")):
                        reminder_text += f" - {format_date_safe(row.get('reminder_date', ''))}"
                table_data.append([
                    Paragraph(pdf_escape(row.get("id", "")), normal_style),
                    Paragraph(pdf_escape(row.get("title", "")), normal_style),
                    Paragraph(pdf_escape(row.get("work_type", "")), normal_style),
                    Paragraph(pdf_escape(row.get("today_execution_order", "Last") or "Last"), normal_style),
                    Paragraph(pdf_escape(row.get("priority_type", "")), normal_style),
                    Paragraph(pdf_escape(row.get("subject", "")), normal_style),
                    Paragraph(pdf_escape(format_date_safe(row.get("critical_due_date", ""))), normal_style),
        ...
        table = Table(
            table_data,
            colWidths=[0.48 * inch, 2.55 * inch, 0.95 * inch, 0.75 * inch, 0.75 * inch, 1.05 * inch, 0.7 * inch, 0.85 * inch, 0.85 * inch],
            repeatRows=1,
        )
        table.setStyle(TableStyle([
            ("BACKGROUND", (0, 0), (-1, 0), colors.lightgrey),
    ...
    if report_df is not None and not report_df.empty and "today_execution_order" in report_df.columns and (report_df["priority_type"].fillna("").astype(str) == "Today").any():
...
cqa4, cqa5, cqa6, cqa6b = st.columns([2, 3, 3, 2])
proj_map = project_name_map()
with cqa4:
    proj_choice_label = st.selectbox("Idea (optional)", list(proj_map.values()), index=0, key=f"quick_project_select_{st.session_state.quick_add_reset_counter}")
with cqa5:
    quick_notes = st.text_input("Quick notes (optional)", key=f"quick_notes_text_{st.session_state.quick_add_reset_counter}")
with cqa6:
...
if st.button("Check / Add"):
    if txt.strip():
        reverse_map = {v: k for k, v in proj_map.items()}
        selected_project_id = reverse_map.get(proj_choice_label, 0)
        st.session_state.pending_add_item = {
            ...
            "notes": quick_notes.strip(),
            "project_id": None if selected_project_id == 0 else selected_project_id,
        }
...
# V18: true compact wrapped tab navigation.
# Use HTML links with query parameters instead of large Streamlit buttons.
# This keeps the file-folder/tab appearance while greatly reducing height and width.
MAIN_TABS = [
    "iPhone Today", "Today List", "Reports / Print", "Weekly Planning",
    "All Items", "Ideas", "Ideas / Concepts", "Fast Reschedule",
    "Subject Manager", "Work Type Manager", "Priority Manager",
    "Assignee Manager", "Completed Items", "Driving List",
]

TAB_SLUGS = {
    "iPhone Today": "iphone_today",
    "Today List": "today_list",
    "Reports / Print": "reports_print",
    "Weekly Planning": "weekly_planning",
    "All Items": "all_items",
    "Ideas": "ideas",
    "Ideas / Concepts": "ideas_concepts",
    "Fast Reschedule": "fast_reschedule",
    "Subject Manager": "subject_manager",
    "Work Type Manager": "work_type_manager",
    "Priority Manager": "priority_manager",
    "Assignee Manager": "assignee_manager",
    "Completed Items": "completed_items",
    "Driving List": "driving_list",
}
SLUG_TO_TAB = {v: k for k, v in TAB_SLUGS.items()}

# Read selected tab from URL so the compact HTML tabs can navigate without using oversized st.button cards.
try:
    selected_slug = st.query_params.get("tab", TAB_SLUGS.get(st.session_state.get("active_main_tab", "iPhone Today"), "iphone_today"))
except Exception:
    selected_slug = TAB_SLUGS.get(st.session_state.get("active_main_tab", "iPhone Today"), "iphone_today")

if isinstance(selected_slug, list):
    selected_slug = selected_slug[0] if selected_slug else "iphone_today"

selected_tab = SLUG_TO_TAB.get(str(selected_slug), st.session_state.get("active_main_tab", "iPhone Today"))
if selected_tab not in MAIN_TABS:
    selected_tab = "iPhone Today"
st.session_state.active_main_tab = selected_tab

st.markdown("""
<style>
/* V18 compact wrapped tabs: real small label tabs, not large button cards. */
.compact-tab-wrap {
    display: flex;
    flex-wrap: wrap;
    gap: 6px 7px;
...
.compact-tab-wrap a.active-tab,
.compact-tab-wrap a.active-tab:visited {
    color: #ffffff !important;
    background: #ff4b4b;
    border-color: #ff4b4b;
}
.compact-tab-wrap a:hover {
    border-color: #ff4b4b;
    color: #ff4b4b !important;
}
.compact-tab-wrap a.active-tab:hover {
    color: #ffffff !important;
}
@media (max-width: 700px) {
    .compact-tab-wrap {
...
for label in MAIN_TABS:
    slug = TAB_SLUGS[label]
    active_class = " active-tab" if label == st.session_state.active_main_tab else ""
    links.append(f'<a class="{active_class.strip()}" href="?tab={slug}">{label}</a>')
st.markdown('<div class="compact-tab-wrap">' + "".join(links) + '</div>', unsafe_allow_html=True)
...
if current_page == "Reports / Print":
    ...
    st.write("Create a printable item report by Work Type, Priority, Subject, Reminder status, and/or Critical Due Date.")
    ...
    if report_df.empty:
        ...
        with rf1:
            report_work_type = st.selectbox("Work Type", ["All"] + get_work_types(), index=0, key="report_work_type_filter")
        with rf2:
            report_priority = st.selectbox("Priority", ["All"] + get_priority_types(), index=0, key="report_priority_filter")
        with rf3:
            report_subject = st.selectbox("Subject", ["All"] + get_subjects(), index=0, key="report_subject_filter")
        with rf4:
            report_reminder_mode = st.selectbox("Reminder", ["All", "Only Reminders", "Exclude Reminders"], index=0, key="report_reminder_mode")
        with rf5:
            critical_due_mode = st.selectbox("Critical Due Date", ["All", "Specific Date", "Only Items With Critical Due Date"], index=0, key="report_critical_due_mode")
            critical_due_filter_date = st.date_input("Choose critical due date", value=date.today(), key="report_critical_due_date", disabled=(critical_due_mode != "Specific Date"))

        sf1, sf2 = st.columns(2)
        with sf1:
            report_sort_by = st.selectbox(
                "Sort By",
                ["Item #", "Work Type", "Subject", "Priority", "Critical Due Date"],
                index=1,
                key="report_sort_by",
            )
        with sf2:
            report_sort_direction = st.selectbox(
                "Sort Direction",
                ["Ascending", "Descending"],
                index=0,
                key="report_sort_direction",
            )
        ...
        if report_priority != "All":
            report_df = report_df[report_df["priority_type"].fillna("As Needed").astype(str).str.strip() == str(report_priority).strip()]
        ...
        sort_map = {
            ...
            "Critical Due Date": "critical_due_date",
        }
        primary_sort_col = sort_map.get(report_sort_by, "work_type")
        sort_cols = ["today_order_sort"] if report_priority == "Today" else []
        if primary_sort_col not in sort_cols:
            sort_cols.append(primary_sort_col)
        for extra_col in ["subject", "work_type", "priority_type", "critical_due_date", "title", "id"]:
        ...
        display_cols = [c for c in ["id", "title", "work_type", "today_execution_order", "priority_type", "subject", "critical_due_date", "is_appointment", "appointment_date", "is_reminder", "reminder_date"] if c in report_df.columns]
        ...
        report_pdf = build_items_report_pdf(report_df, report_title="Task Flow Items Report", filter_summary=filter_summary)
        st.download_button(
            ...
            key="download_filtered_report_pdf",
        )
...
if current_page == "Weekly Planning":
    ...
    if weekly_df.empty:
        ...
        backlog_only = [r for r in weekly_df.to_dict("records") if r.get("priority_type", "As Needed") == "Scan / File"]
...
if current_page == "All Items":
    ...
    if df_all.empty:
        ...
        if filter_priority != "All":
            df_all = df_all[df_all["priority_type"].fillna("As Needed") == filter_priority]
        ...
        for r in df_all.itertuples():
            with st.container(border=True):
                ...
                with ec4:
                    current_priority = r.priority_type if r.priority_type in priority_choices_all else "As Needed"
    ...
    if filtered_projects_df.empty:
        ...
        if pha1.button("Save Idea", key=f"save_project_header_{selected_project_id}"):
            save_item(
                ...
                project_row["priority_type"] if pd.notna(project_row["priority_type"]) else "As Needed",
...
if current_page == "Priority Manager":
    ...
    with pm2:
        ...
        default_replacement_index = replacement_choices.index("As Needed") if "As Needed" in replacement_choices else 0
        ...
        if st.button("Delete Priority"):
            ok, msg = delete_priority_type(delete_priority_name, replacement_priority if replacement_choices else "As Needed")
            if ok:
                st.success(msg)
            else:
                st.warning(msg)
...
if current_page == "Assignee Manager":
    ...
    st.write("Create assignees, assign items, and print assignee work lists.")
    ...
    assign_df = load_all().copy()
    if assign_df.empty:
        st.info("No open items available to assign.")
    elif not get_assignee_options(include_unassigned=False):
        st.info("Create at least one assignee before assigning items.")
    else:
        af1, af2, af3, af4 = st.columns(4)
        with af1:
            assign_priority = st.selectbox("Filter by Priority", ["All"] + get_priority_types(), index=0, key="assign_filter_priority")
        with af2:
            assign_subject = st.selectbox("Filter by Subject", ["All"] + get_subjects(), index=0, key="assign_filter_subject")
        with af3:
            assign_current_assignee = st.selectbox("Filter by Current Assignee", ["All"] + get_assignee_options(include_unassigned=True), index=0, key="assign_filter_current_assignee")
        with af4:
            assign_search = st.text_input("Search task text", key="assign_filter_search")

        if assign_priority != "All":
            assign_df = assign_df[assign_df["priority_type"].fillna("As Needed") == assign_priority].copy()
        if assign_subject != "All":
            assign_df = assign_df[assign_df["subject"].fillna("Unassigned") == assign_subject].copy()
        if assign_current_assignee != "All":
            current_assignee_id = assignee_name_to_id(assign_current_assignee)
            if assign_current_assignee == "Unassigned":
                assign_df = assign_df[assign_df["assignee_id"].isna()].copy()
            else:
                assign_df = assign_df[assign_df["assignee_id"] == current_assignee_id].copy()
        if assign_search.strip():
            q = assign_search.strip().lower()
            assign_df = assign_df[assign_df.apply(lambda row: q in str(row.get("title", "")).lower() or q in str(row.get("notes", "")).lower() or q in str(row.get("subject", "")).lower(), axis=1)].copy()

        st.caption(f"Assignable items shown: {len(assign_df)}")
        if assign_df.empty:
            st.info("No items match those assignment filters.")
        else:
            assign_df = assign_df.sort_values(by=["priority_type", "subject", "id"], ascending=[True, True, True]).copy()
            label_to_id = {}
            item_labels = []
            for _, row in assign_df.iterrows():
                assignee_label = assignee_id_to_name(row.get("assignee_id", None))
                label = f"#{int(row['id'])} | {row.get('priority_type', 'As Needed')} | {row.get('subject', 'Unassigned')} | Assigned: {assignee_label} | {str(row.get('title', '') or '')[:140]}"
                label_to_id[label] = int(row["id"])
                item_labels.append(label)

            selected_assignment_labels = st.multiselect("Select items to assign / unassign", item_labels, key="assign_selected_items")
            target_assignee = st.selectbox("Assign selected items to", get_assignee_options(include_unassigned=True), index=0, key="assign_target_assignee")
            if st.button("Apply Assignment", key="apply_assignment_button"):
                selected_ids = [label_to_id[label] for label in selected_assignment_labels]
                if not selected_ids:
                    st.warning("Select at least one item.")
                else:
                    assign_items_to_assignee(selected_ids, target_assignee)
                    st.success(f"Updated {len(selected_ids)} item(s).")
                    st.rerun()


    st.markdown("---")
    st.markdown("### Build Work List Fast")
    st.write("Use this faster section to pick items, edit the worker-facing instructions inline, and print a PDF from the same place.")

    if not get_assignee_options(include_unassigned=False):
        st.info("Create at least one assignee before building a work list.")
    else:
        wb1, wb2, wb3, wb4 = st.columns(4)
        with wb1:
            worklist_assignee = st.selectbox("Work List Assignee", get_assignee_options(include_unassigned=False), index=0, key="worklist_builder_assignee")
        with wb2:
            worklist_priority = st.selectbox("Work List Priority", ["All"] + get_priority_types(), index=0, key="worklist_builder_priority")
        with wb3:
            worklist_subject = st.selectbox("Work List Subject", ["All"] + get_subjects(), index=0, key="worklist_builder_subject")
        with wb4:
            worklist_search = st.text_input("Search Work List Items", key="worklist_builder_search")

        wb5, wb6 = st.columns(2)
        with wb5:
            worklist_current_assignee = st.selectbox("Current Assignment Filter", ["All", "Unassigned", "Already Assigned to This Person"] + get_assignee_options(include_unassigned=False), index=0, key="worklist_builder_current_assignment")
        with wb6:
            worklist_limit = st.selectbox("Rows to show in builder", [25, 50, 100, 200, 500], index=2, key="worklist_builder_limit")

        worklist_df = load_all().copy()
        if worklist_priority != "All":
            worklist_df = worklist_df[worklist_df["priority_type"].fillna("As Needed") == worklist_priority].copy()
        if worklist_subject != "All":
            worklist_df = worklist_df[worklist_df["subject"].fillna("Unassigned") == worklist_subject].copy()
        if worklist_current_assignee == "Unassigned":
            worklist_df = worklist_df[worklist_df["assignee_id"].isna()].copy()
        elif worklist_current_assignee == "Already Assigned to This Person":
            this_assignee_id = assignee_name_to_id(worklist_assignee)
            worklist_df = worklist_df[worklist_df["assignee_id"] == this_assignee_id].copy()
        elif worklist_current_assignee not in ("All", "Unassigned", "Already Assigned to This Person"):
            selected_current_assignee_id = assignee_name_to_id(worklist_current_assignee)
            worklist_df = worklist_df[worklist_df["assignee_id"] == selected_current_assignee_id].copy()
        if worklist_search.strip():
            q = worklist_search.strip().lower()
            worklist_df = worklist_df[worklist_df.apply(
                lambda row: q in str(row.get("title", "")).lower()
                or q in str(row.get("notes", "")).lower()
                or q in str(row.get("subject", "")).lower()
                or q in str(row.get("assignee_task_text", "")).lower(),
                axis=1
            )].copy()

        worklist_df = worklist_df.sort_values(by=["priority_type", "subject", "id"], ascending=[True, True, True]).head(int(worklist_limit)).copy()
        st.caption(f"Work list builder rows shown: {len(worklist_df)}")

        if worklist_df.empty:
            st.info("No items matched the work list builder filters.")
        else:
            builder_rows = []
            for _, row in worklist_df.iterrows():
                appt_text = format_date_safe(row.get("appointment_date", "")) if bool_from_db(row.get("is_appointment", 0)) else ""
                rem_text = format_date_safe(row.get("reminder_date", "")) if bool_from_db(row.get("is_reminder", 0)) else ""
                saved_task_text = str(row.get("assignee_task_text", "") or "").strip()
                title_text = str(row.get("title", "") or "")
                builder_rows.append({
                    "Include": False,
                    "Item ID": int(row.get("id")),
                    "Task Text": saved_task_text if saved_task_text else title_text,
                    "Subject": str(row.get("subject", "") or "Unassigned"),
                    "Priority": str(row.get("priority_type", "") or "As Needed"),
                    "Critical Due Date": format_date_safe(row.get("critical_due_date", "")),
                    "Appointment": appt_text,
                    "Reminder": rem_text,
                    "Current Assignee": assignee_id_to_name(row.get("assignee_id", None)),
                    "Master Item Title": title_text,
                })

            builder_editor_df = pd.DataFrame(builder_rows)
            edited_worklist_df = st.data_editor(
                builder_editor_df,
                key="worklist_builder_editor",
                hide_index=True,
                use_container_width=True,
                num_rows="fixed",
                row_height=84,
                disabled=["Item ID", "Subject", "Priority", "Critical Due Date", "Appointment", "Reminder", "Current Assignee", "Master Item Title"],
                column_config={
                    "Include": st.column_config.CheckboxColumn("Include", width="small"),
                    "Item ID": st.column_config.NumberColumn("Item ID", width="small"),
                    "Task Text": st.column_config.TextColumn("Task Text", width="large"),
                    "Subject": st.column_config.TextColumn("Subject", width="medium"),
                    "Priority": st.column_config.TextColumn("Priority", width="small"),
                    "Critical Due Date": st.column_config.TextColumn("Critical Due Date", width="small"),
                    "Appointment": st.column_config.TextColumn("Appointment", width="small"),
                    "Reminder": st.column_config.TextColumn("Reminder", width="small"),
                    "Current Assignee": st.column_config.TextColumn("Current Assignee", width="medium"),
                    "Master Item Title": st.column_config.TextColumn("Master Item Title", width="large"),
                },
            )

            selected_worklist_df = edited_worklist_df[edited_worklist_df["Include"] == True].copy()
            if not selected_worklist_df.empty:
                selected_worklist_df["Assignee"] = worklist_assignee
                selected_worklist_df = selected_worklist_df[["Assignee", "Item ID", "Task Text", "Subject", "Priority", "Critical Due Date", "Appointment", "Reminder"]].copy()

            st.caption(f"Selected for current work list: {len(selected_worklist_df)} item(s).")

            bc1, bc2 = st.columns(2)
            with bc1:
                if st.button("Save Selected Assignments / Task Text", key="worklist_builder_save_assignments"):
                    if selected_worklist_df.empty:
                        st.warning("Select at least one item to save.")
                    else:
                        target_assignee_id = assignee_name_to_id(worklist_assignee)
                        with conn() as c:
                            cur = c.cursor()
                            for _, row in selected_worklist_df.iterrows():
                                cur.execute(
                                    "UPDATE items SET assignee_id=?, assignee_task_text=? WHERE id=?",
                                    (target_assignee_id, str(row.get("Task Text", "") or "").strip(), int(row.get("Item ID")))
                                )
                            c.commit()
                        invalidate_caches()
                        st.success(f"Saved {len(selected_worklist_df)} item(s) to {worklist_assignee}.")
                        st.rerun()

            with bc2:
                if selected_worklist_df.empty:
                    st.info("Select items above to generate a PDF.")
                else:
                    worklist_pdf = build_assignee_worklist_pdf(
                        selected_worklist_df,
                        report_title=f"Work List - {worklist_assignee}",
                    )
                    st.download_button(
                        "Download / Print Current Work List PDF",
                        data=worklist_pdf,
                        file_name=f"work_list_{worklist_assignee.replace(' ', '_').replace('&', 'and')}.pdf",
                        mime="application/pdf",
                        key="download_fast_worklist_pdf",
                    )

    st.markdown("---")
    st.markdown("### Edit Assignee Task Text")
    st.write("Use this to shorten or customize the exact instruction that prints on the assignee work list. By default, only assigned items are shown. If left blank, the report uses the original item title.")

    edit_show_unassigned = st.checkbox("Show Unassigned Items", value=False, key="edit_assignee_text_show_unassigned")
    ...
    edit_df = pd.read_sql("""
        ...
        LEFT JOIN assignees ON items.assignee_id = assignees.id
        WHERE COALESCE(items.item_type, 'Task') NOT IN ('Project', 'Ideas')
          AND COALESCE(items.state, 'Open') != 'Complete'
        ORDER BY COALESCE(assignees.name, 'Unassigned'), items.priority_type, items.subject, items.id
    """, conn())

    if not edit_show_unassigned and not edit_df.empty:
        edit_df = edit_df[edit_df["assignee_id"].notna()].copy()

    if edit_df.empty:
        st.info("No open items available for assignee task text editing.")
    else:
        et1, et2, et3 = st.columns(3)
        with et1:
            edit_assignee_filter = st.selectbox("Filter Assignee Task Text by Assignee", ["All"] + get_assignee_options(include_unassigned=True), index=0, key="edit_assignee_text_filter")
        with et2:
            edit_priority_filter = st.selectbox("Filter Assignee Task Text by Priority", ["All"] + get_priority_types(), index=0, key="edit_assignee_text_priority")
        with et3:
            edit_subject_filter = st.selectbox("Filter Assignee Task Text by Subject", ["All"] + get_subjects(), index=0, key="edit_assignee_text_subject")

        if edit_assignee_filter != "All":
            if edit_assignee_filter == "Unassigned":
                edit_df = edit_df[edit_df["assignee_id"].isna()].copy()
            else:
                edit_df = edit_df[edit_df["assignee_name"].fillna("Unassigned") == edit_assignee_filter].copy()
        if edit_priority_filter != "All":
            edit_df = edit_df[edit_df["priority_type"].fillna("As Needed") == edit_priority_filter].copy()
        if edit_subject_filter != "All":
            edit_df = edit_df[edit_df["subject"].fillna("Unassigned") == edit_subject_filter].copy()

        st.caption(f"Items shown for assignee text editing: {len(edit_df)}")
        if edit_df.empty:
            st.info("No items matched those filters.")
        else:
            for _, row in edit_df.iterrows():
                item_id = int(row["id"])
                assignee_label = assignee_id_to_name(row.get("assignee_id", None))
                title_text = str(row.get("title", "") or "")
                current_assignee_text = str(row.get("assignee_task_text", "") or "")
                with st.container(border=True):
                    st.caption(f"Item #{item_id} | Assignee: {assignee_label} | Subject: {row.get('subject', 'Unassigned')} | Priority: {row.get('priority_type', 'As Needed')}")
                    st.write(f"Master Item Title: {title_text}")
                    new_assignee_task_text = st.text_area(
                        f"Assignee Task Text #{item_id}",
                        value=current_assignee_text if current_assignee_text.strip() else title_text,
                        height=title_text_area_height(current_assignee_text if current_assignee_text.strip() else title_text),
                        key=f"assignee_task_text_{item_id}",
                    )
                    if st.button("Save Assignee Task Text", key=f"save_assignee_task_text_{item_id}"):
                        save_assignee_task_text(item_id, new_assignee_task_text)
                        st.success(f"Saved assignee task text for item #{item_id}.")
                        st.rerun()

    st.markdown("---")
    st.markdown("### Assignee Work List / Print")
    rp1, rp2, rp3, rp4 = st.columns(4)
    with rp1:
        report_assignee = st.selectbox("Assignee", ["All"] + get_assignee_options(include_unassigned=True), index=0, key="assignee_report_assignee")
    with rp2:
        report_assignee_priority = st.selectbox("Priority", ["All"] + get_priority_types(), index=0, key="assignee_report_priority")
    with rp3:
        report_assignee_subject = st.selectbox("Subject", ["All"] + get_subjects(), index=0, key="assignee_report_subject")
    with rp4:
        report_include_completed = st.checkbox("Include completed", value=False, key="assignee_report_include_completed")

    assignee_report_df = build_assignee_report_df(
        assignee_name=report_assignee,
        priority_filter=report_assignee_priority,
        subject_filter=report_assignee_subject,
        include_completed=report_include_completed,
    )

    display_cols = ["Assignee", "Item ID", "Task Text", "Subject", "Priority", "Critical Due Date", "Appointment", "Reminder"]
    st.caption(f"Report rows: {len(assignee_report_df)}")
    if assignee_report_df.empty:
        st.info("No items matched the selected assignee report filters.")
    else:
        st.dataframe(assignee_report_df[display_cols], use_container_width=True, hide_index=True)
        assignee_pdf = build_assignee_worklist_pdf(
            assignee_report_df,
            report_title=f"Assignee Work List - {report_assignee}",
        )
        st.download_button(
            "Download / Print Assignee Work List PDF",
            data=assignee_pdf,
            file_name="assignee_work_list.pdf",
            mime="application/pdf",
            key="download_assignee_work_list_pdf",
        )
