...
if current_page == "Reports / Print":
    ...
    if report_df.empty:
        ...
        with df1:
            date_entered_start_mode = st.selectbox(
                "Start Date",
                ["None", "Use Start Date"],
                index=0,
                key="report_created_start_mode_v27",
            )
        with df2:
            date_entered_start_date = st.date_input(
                "Choose start date",
                value=date.today(),
                key="report_created_start_date_v27",
                disabled=(date_entered_start_mode == "None"),
            )
        with df3:
            date_entered_end_mode = st.selectbox(
                "End Date",
                ["None", "Use End Date"],
                index=0,
                key="report_created_end_mode_v27",
            )
        with df4:
            date_entered_end_date = st.date_input(
                "Choose end date",
                value=date.today(),
                key="report_created_end_date_v27",
                disabled=(date_entered_end_mode == "None"),
            )

        sf1, sf2 = st.columns(2)
        with sf1:
            # V27: Date Entered is a true sort option and can be combined with Subject/Priority filters.
            report_sort_options = ["Date Entered", "Subject", "Item #", "Work Type", "Priority", "Critical Due Date"]
            report_sort_by = st.selectbox(
                "Sort By",
                report_sort_options,
                index=0,
                key="report_sort_by_v27_date_entered_true",
            )
        with sf2:
        ...
        # Date Entered filter. Both ends are optional:
        # - Start None + End None = all dates
        # - Start Date + End None = from start date forward
        # - Start None + End Date = everything through end date
        ...
        report_pdf = build_items_report_pdf(report_df, report_title="Task Flow Items Report", filter_summary=filter_summary, include_today_order=include_today_order_report)
        st.download_button(
