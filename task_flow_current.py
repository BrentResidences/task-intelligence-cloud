...
def get_session_pg_connection():
    ...
    Opening a new cloud database connection for every small button click is slow.
    This keeps Quick Add and Today-list actions noticeably faster while still
    reconnecting automatically if Neon closes the connection.
    """
    if not DATABASE_URL:
...
def delete_item(item_id):
    with conn() as c:
        # If this item is a project header, keep child items but detach them from the deleted project.
        c.execute("UPDATE items SET project_id=NULL WHERE project_id=?", (item_id,))
        ...
        with st.container(border=True):
            ...
            with a3:
                if st.button("Complete", key=f"{prefix}_complete_{r.id}"):
                    ...
                    st.rerun()
            with a4:
                confirm_delete = st.checkbox("Confirm delete", key=f"{prefix}_confirm_delete_{r.id}")
                if st.button("Delete Item", key=f"{prefix}_delete_{r.id}", disabled=not confirm_delete):
                    delete_item(r.id)
                    st.rerun()
...
with qa_ap3:
    quick_is_reminder = st.checkbox("Reminder", key=f"quick_is_reminder_{st.session_state.quick_add_reset_counter}")
with qa_ap4:
...
tab0, tab1, tab14, tab2, tab3, tab4, tab5, tab6, tab8, tab9, tab13, tab15, tab10, tab12 = st.tabs(["iPhone Today", "Today List", "Reports / Print", "Weekly Planning", "All Items", "Ideas", "Ideas / Concepts", "Fast Reschedule", "Subject Manager", "Work Type Manager", "Priority Manager", "Assignee Manager", "Completed Items", "Driving List"])
...
with tab14:
    ...
    st.write("Create a printable item report by Work Type, Priority, Subject, and/or Critical Due Date.")
    ...
    if report_df.empty:
        ...
        filter_summary = (
            f"Work Type: {report_work_type} | Priority: {report_priority} | Subject: {report_subject} | Critical Due Date: "
    ...
    if df_all.empty:
        ...
        for r in df_all.itertuples():
            with st.container(border=True):
                ...
                new_item_type = r.item_type if str(r.item_type or "").strip() else "Task"
                with ec1:
                    current_work_type = r.work_type if r.work_type in get_work_types() else "Unassigned"
                    new_work_type = st.selectbox(f"Work Type #{r.id}", get_work_types(), index=get_work_types().index(current_work_type), key=f"wt_{r.id}")
                with ec2:
                    assignee_options = get_assignee_options(include_unassigned=True)
                    current_assignee_name = assignee_id_to_name(getattr(r, "assignee_id", None))
                    if current_assignee_name not in assignee_options:
                        current_assignee_name = "Unassigned"
                    new_assignee_name = st.selectbox(f"Assignee #{r.id}", assignee_options, index=assignee_options.index(current_assignee_name), key=f"assignee_{r.id}")
                with ec3:
                    current_subject = r.subject if r.subject in get_subjects() else "Unassigned"
                    new_subject = st.selectbox(f"Subject #{r.id}", get_subjects(), index=get_subjects().index(current_subject), key=f"subj_{r.id}")
                with ec4:
                    priority_choices = get_priority_types()
                    current_priority = r.priority_type if r.priority_type in priority_choices else "As Needed"
                    new_priority_type = st.selectbox(f"Priority #{r.id}", priority_choices, index=priority_choices.index(current_priority) if current_priority in priority_choices else 0, key=f"prio_{r.id}")

                notes_val = st.text_input(f"Notes #{r.id}", value=r.notes if pd.notna(r.notes) else "", key=f"notes_{r.id}")
                project_select = st.selectbox(f"Idea #{r.id}", list(current_proj_map.values()), index=list(current_proj_map.values()).index(project_label) if project_label in current_proj_map.values() else 0, key=f"proj_{r.id}")
                critical_due_value = clearable_date_input(
                    f"Critical Due Date #{r.id} (MM/DD/YYYY)",
                    parse_date_safe(getattr(r, "critical_due_date", "")),
                ...
                if action3.button("Reopen", key=f"reopen_{r.id}"):
                    ...
                    st.rerun()
                with action4:
                    confirm_delete = st.checkbox("Confirm delete", key=f"all_items_confirm_delete_{r.id}")
                    if st.button("Delete Item", key=f"all_items_delete_{r.id}", disabled=not confirm_delete):
                        delete_item(r.id)
                        st.rerun()
...
with tab4:
    ...
    if filtered_projects_df.empty:
        ...
        if pha3.button("Reopen Idea", key=f"reopen_project_header_{selected_project_id}"):
            ...
            st.rerun()
        with pha4:
            confirm_project_delete = st.checkbox("Confirm delete", key=f"confirm_delete_idea_header_{selected_project_id}")
            if st.button("Delete Idea", key=f"delete_idea_header_{selected_project_id}", disabled=not confirm_project_delete):
        ...
        if children_df.empty:
            ...
            for r in children_df.itertuples():
                ...
                if c3.button("Complete", key=f"child_complete_{r.id}"):
                    ...
                    st.rerun()
                with c4:
                    confirm_child_delete = st.checkbox("Confirm", key=f"child_confirm_delete_{r.id}")
                    if st.button("Delete", key=f"child_delete_{r.id}", disabled=not confirm_child_delete):
                        delete_item(r.id)
                        st.rerun()
