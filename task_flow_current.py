...
# Version 31.0: adds a condensed Task Repository PDF with Item #, Subject, and Title only.
# Version 30.2: makes the Detailed and Condensed report choices explicitly visible.
# Version 30.0: grouped navigation, Active Tasks terminology, Fast Reschedule removal, and two-way Task Repository movement.
# Version 29.0: Task Repository and Concept Repository redesign.
# Version 28.0: cloud stability repair. Pandas reads now use a proper SQLAlchemy
# connection instead of the custom psycopg2 compatibility wrapper. The interface,
# database schema, pages, and workflows are intentionally unchanged.
try:
...
APP_LOGGER = logging.getLogger("task_intelligence")

# Version 28.0: pandas SELECT queries use SQLAlchemy with fresh, short-lived connections; page-entry and native-fault diagnostics added.
...
def get_read_engine():
    """Return a thread-safe SQLAlchemy engine used only for pandas reads.

    NullPool intentionally avoids retaining or sharing a native psycopg2 connection
    between Streamlit reruns. Every read obtains a fresh connection and closes it
    immediately after the DataFrame is created.
    """
    if not DATABASE_URL:
    ...
    return create_engine(
        ...
        connect_args={
            ...
            "keepalives_count": 3,
            "application_name": "task_intelligence_sqlalchemy_reads",
        },
    )


def _convert_qmark_query(sql, params=None):
    """Convert the app's SQLite-style ? placeholders to SQLAlchemy parameters."""
    sql = str(sql)
    if params is None:
        return sql, {}
    ...
    for index, value in enumerate(values):
        ...
        bound[f"p{index}"] = value
    return sql, bound


def read_sql_df(sql, params=None):
    """Run a SELECT through SQLAlchemy and return a fully detached DataFrame."""
    converted_sql, bound_params = _convert_qmark_query(sql, params)
    try:
        with get_read_engine().connect() as db_connection:
        ...
        APP_LOGGER.exception("DATABASE_READ_FAILED sql=%s", converted_sql[:500])
        raise


def log_page_entry(page_name):
    APP_LOGGER.info("PAGE_ENTER page=%s pid=%s", page_name, os.getpid())
...
def get_subjects():
    df = read_sql_df("SELECT name FROM subjects ORDER BY LOWER(name)")
    vals = ["Unassigned"]
    if not df.empty:
...
@st.cache_data(show_spinner=False)
def get_work_types():
    df = read_sql_df("SELECT name FROM work_types ORDER BY LOWER(name)")
    vals = ["Unassigned"]
    if not df.empty:
        vals.extend(df["name"].tolist())
    return vals
...
def get_priority_types():
    df = read_sql_df("SELECT name FROM priority_types ORDER BY id")
    vals = []
    if not df.empty:
        vals.extend(df["name"].tolist())
    if not vals:
...
def get_assignees_df(include_inactive=False):
    if include_inactive:
        df = read_sql_df("SELECT id, name, active FROM assignees ORDER BY LOWER(name)")
    else:
        df = read_sql_df("SELECT id, name, active FROM assignees WHERE COALESCE(active, 1)=1 ORDER BY LOWER(name)")
    return df
...
def generate(n=5, include_project_filler=True):
    df = read_sql_df("SELECT * FROM items WHERE in_today=0 AND COALESCE(state, 'Open') != 'Complete' AND COALESCE(item_type, 'Task') NOT IN ('Project', 'Ideas')")
    if df.empty:
        return
...
@st.cache_data(show_spinner=False)
def load_all():
    return read_sql_df("SELECT * FROM items WHERE COALESCE(state, 'Open') != 'Complete' AND COALESCE(item_type, 'Task') NOT IN ('Project', 'Ideas') ORDER BY id DESC")


@st.cache_data(show_spinner=False)
def load_completed():
    return read_sql_df("SELECT * FROM items WHERE COALESCE(state, 'Open') = 'Complete' ORDER BY id DESC")


@st.cache_data(show_spinner=False)
def load_projects():
    return read_sql_df("SELECT * FROM items WHERE item_type IN ('Project', 'Ideas') AND COALESCE(state, 'Open') != 'Complete' AND (project_id IS NULL OR project_id=0) ORDER BY id DESC")


@st.cache_data(show_spinner=False)
def load_project_children(project_id):
    return read_sql_df("SELECT * FROM items WHERE project_id=? AND COALESCE(state, 'Open') != 'Complete' ORDER BY id DESC", params=(project_id,))


@st.cache_data(show_spinner=False)
def load_projects_and_concepts():
    return read_sql_df("SELECT * FROM items WHERE COALESCE(state, 'Open') != 'Complete' AND ((item_type IN ('Project', 'Ideas') AND (project_id IS NULL OR project_id=0)) OR item_type='Concept') ORDER BY item_type, id DESC")
...
def weekly_bucketed_rows():
    weekly_df = load_weekly_queue()
    if weekly_df.empty:
        return {"Urgent / Overdue": [], "Due This Week": [], "Repository Review Attention This Week": [], "Scan / File / Fill Work": []}
    ...
    return {
        ...
        "Repository Review Attention This Week": review_rows,
...
def add_top_weekly_items_to_today(max_items=5):
    ...
    for bucket_name in ["Urgent / Overdue", "Due This Week", "Repository Review Attention This Week", "Scan / File / Fill Work"]:
...
@st.cache_data(show_spinner=False)
def load_task_repository():
    """Load legacy project/idea records as Task Repository entries."""
    return read_sql_df("""
        SELECT *
        FROM items
        WHERE COALESCE(state, 'Open') != 'Complete'
          AND item_type IN ('Project', 'Ideas')
...
def invalidate_repository_caches():
    for fn_name in [
        "load_task_repository",
        "load_concept_repository",
        "load_projects",
        "load_projects_and_concepts",
        "project_name_map",
        "load_all",
...
def copy_repository_item_to_active(item_id):
    row_df = read_sql_df("SELECT * FROM items WHERE id=?", params=(int(item_id),))
    if row_df.empty:
        return False, "Repository item was not found."
    row = row_df.iloc[0]
    add_item(
        str(row.get("title", "") or "").strip(),
        item_type="Task",
        work_type=str(row.get("work_type", "") or "Unassigned"),
        subject=str(row.get("subject", "") or "Unassigned"),
        priority_type=str(row.get("priority_type", "") or "Task Repository"),
        notes=str(row.get("notes", "") or ""),
        project_id=None,
        due=str(row.get("due", "") or ""),
        review_date=str(row.get("review_date", "") or ""),
        review_start=str(row.get("review_start", "") or ""),
        review_end=str(row.get("review_end", "") or ""),
        is_appointment=int(bool_from_db(row.get("is_appointment", 0))),
        appointment_date=str(row.get("appointment_date", "") or ""),
        is_reminder=int(bool_from_db(row.get("is_reminder", 0))),
        reminder_date=str(row.get("reminder_date", "") or ""),
        critical_due_date=str(row.get("critical_due_date", "") or ""),
    )
    invalidate_repository_caches()
    return True, "Copied to the active task list."


def move_repository_item_to_active(item_id):
    with conn() as c:
        c.execute(
            """
            UPDATE items
            SET item_type='Task', project_id=NULL, state='Open'
            WHERE id=?
            """,
            (int(item_id),),
        )
    ...
    invalidate_item_caches()
    invalidate_repository_caches()
    return True, "Moved to the active task list."


def move_active_task_to_repository(item_id):
    """Keep an active task but remove it from all active-task views."""
    with conn() as c:
        c.execute(
            """
            UPDATE items
            SET item_type='Ideas', project_id=NULL, state='Open', in_today=0
            WHERE id=?
            """,
            (int(item_id),),
        )
    ...
    invalidate_item_caches()
    invalidate_repository_caches()
    return True, "Moved to the Task Repository."


def build_repository_print_pdf(repository_df, repository_name, filter_summary=""):
    buffer = BytesIO()
    doc = SimpleDocTemplate(
        buffer,
        pagesize=letter,
    ...
    story = [Paragraph(pdf_escape(f"{repository_name} Report"), title_style)]
    if filter_summary:
        story.append(Spacer(1, 0.08 * inch))
        story.append(Paragraph(pdf_escape(filter_summary), styles["Normal"]))
    story.append(Spacer(1, 0.10 * inch))
    story.append(Paragraph(f"Rows printed: {0 if repository_df is None else len(repository_df)}", styles["Normal"]))
    story.append(Spacer(1, 0.12 * inch))

    table_data = [[
        Paragraph("<b>Item #</b>", normal_style),
        Paragraph("<b>Title</b>", normal_style),
        Paragraph("<b>Work Type</b>", normal_style),
        Paragraph("<b>Subject</b>", normal_style),
        Paragraph("<b>Date Entered</b>", normal_style),
        Paragraph("<b>Notes / Description</b>", normal_style),
    ]]

    if repository_df is not None and not repository_df.empty:
    ...
    table = Table(
        table_data,
        colWidths=[0.5 * inch, 2.05 * inch, 0.95 * inch, 1.25 * inch, 0.75 * inch, 2.0 * inch],
        repeatRows=1,
    )
    table.setStyle(TableStyle([
        ("BACKGROUND", (0, 0), (-1, 0), colors.lightgrey),
...
def build_condensed_repository_report_pdf(repository_df, repository_name="Task Repository", filter_summary=""):
    """Create a compact landscape repository report with Item #, Subject, and Title only."""
    buffer = BytesIO()
    doc = SimpleDocTemplate(
        buffer,
        pagesize=landscape(letter),
    ...
    story = [Paragraph(pdf_escape(f"Condensed {repository_name} Report"), title_style)]
    if filter_summary:
    ...
    if repository_df is not None and not repository_df.empty:
        for _, row in repository_df.iterrows():
            table_data.append([
                Paragraph(pdf_escape(row.get("id", "")), normal_style),
                Paragraph(pdf_escape(row.get("subject", "") or "Unassigned"), normal_style),
                Paragraph(pdf_escape(row.get("title", "")), normal_style),
            ])

    table = Table(
        table_data,
        colWidths=[0.60 * inch, 1.65 * inch, 8.25 * inch],
        repeatRows=1,
        splitByRow=1,
    )
    table.setStyle(TableStyle([
...
def render_repository_page(repository_name, repository_item_type, allow_active_conversion):
    st.subheader(repository_name)
    if repository_name == "Task Repository":
        st.write("Capture possible things to do here before adding them to the active task list.")
        source_df = load_task_repository()
    else:
        st.write("Capture broad, project-oriented thoughts and generalized concepts here.")
        source_df = load_concept_repository()

    reset_key = f"{repository_name.lower().replace(' ', '_')}_create_reset"
    if reset_key not in st.session_state:
        st.session_state[reset_key] = 0
    reset_counter = st.session_state[reset_key]

    repository_work_types = [w for w in get_work_types() if w != "Ideas"]
    if not repository_work_types:
        repository_work_types = ["Unassigned"]

    st.markdown(f"### Quick Add to {repository_name}")
    qa1, qa2, qa3 = st.columns([5, 2, 2])
    with qa1:
        new_title = st.text_area(
            f"{repository_name} entry",
            height=105,
            key=f"{reset_key}_title_{reset_counter}",
        )
    with qa2:
        new_work_type = st.selectbox(
            "Work Type",
            repository_work_types,
            index=0,
            key=f"{reset_key}_work_type_{reset_counter}",
        )
    with qa3:
        new_subject = st.selectbox(
            ...
            get_subjects(),
            index=0,
            key=f"{reset_key}_subject_{reset_counter}",
        )
    new_notes = st.text_area(
        "Notes / description",
        height=90,
        key=f"{reset_key}_notes_{reset_counter}",
    )
    if st.button(f"Add to {repository_name}", key=f"{reset_key}_add_{reset_counter}", type="primary"):
        if new_title.strip():
            add_item(
                new_title.strip(),
                item_type=repository_item_type,
                work_type=new_work_type,
                subject=new_subject,
                priority_type="Task Repository",
            ...
            st.warning("Enter a title before adding the item.")

    st.markdown("---")
    st.markdown(f"### Search and Review {repository_name}")

    f1, f2, f3, f4 = st.columns([3, 2, 2, 2])
    with f1:
        search_text = st.text_input(
            f"Search {repository_name}",
            key=f"{reset_key}_search",
        )
    with f2:
        work_type_filter = st.selectbox(
            "Filter by Work Type",
            ["All"] + repository_work_types,
            index=0,
            key=f"{reset_key}_filter_work_type",
        )
    with f3:
        subject_filter = st.selectbox(
            ...
            ["All"] + get_subjects(),
            index=0,
            key=f"{reset_key}_filter_subject",
        )
    with f4:
        sort_order = st.selectbox(
            ...
            index=0,
            key=f"{reset_key}_sort",
        )

    filtered_df = source_df.copy()
    if not filtered_df.empty and search_text.strip():
        filtered_df = search_items_df(filtered_df, search_text)
    if not filtered_df.empty and work_type_filter != "All":
        filtered_df = filtered_df[filtered_df["work_type"].fillna("Unassigned") == work_type_filter].copy()
    if not filtered_df.empty and subject_filter != "All":
        filtered_df = filtered_df[filtered_df["subject"].fillna("Unassigned") == subject_filter].copy()

    if not filtered_df.empty:
        filtered_df["created_sort"] = filtered_df["created"].apply(report_sort_date)
        filtered_df = filtered_df.sort_values(
            by=["created_sort", "id"],
            ascending=(sort_order == "Oldest to Newest"),
        ).drop(columns=["created_sort"], errors="ignore")

    st.caption(f"Matching entries: {len(filtered_df)}")

    filter_summary = (
        f"Search: {search_text.strip() or 'All'} | Work Type: {work_type_filter} | "
        f"Subject: {subject_filter} | Sort: {sort_order}"
    )
    if repository_name == "Task Repository":
        repository_report_type = st.radio(
            "Choose Task Repository Report Type",
            ["Detailed Report", "Condensed Task Repository Report"],
            index=0,
            key=f"{reset_key}_report_type",
            horizontal=True,
        )
        if repository_report_type == "Condensed Task Repository Report":
            report_pdf = build_condensed_repository_report_pdf(
                filtered_df,
                repository_name=repository_name,
                filter_summary=filter_summary,
            )
            report_button_label = "Download / Print Condensed Task Repository Report PDF"
            report_file_name = "condensed_task_repository_report.pdf"
        else:
            report_pdf = build_repository_print_pdf(filtered_df, repository_name, filter_summary)
            report_button_label = "Download / Print Detailed Task Repository Report PDF"
            report_file_name = "task_repository_report.pdf"
    else:
        report_pdf = build_repository_print_pdf(filtered_df, repository_name, filter_summary)
        report_button_label = f"Download / Print {repository_name} Report PDF"
        report_file_name = f"{repository_name.lower().replace(' ', '_')}_report.pdf"
    ...
    st.download_button(
        ...
        key=f"{reset_key}_report_pdf",
    )

    if filtered_df.empty:
        st.info(f"No {repository_name.lower()} entries matched the selected search and filters.")
        return

    work_types = repository_work_types
    ...
    for row in filtered_df.itertuples():
        with st.container(border=True):
            edited_title = st.text_area(
                f"Title #{row.id}",
                value=str(row.title or ""),
                height=title_text_area_height(row.title),
                key=f"{reset_key}_edit_title_{row.id}",
            )
            e1, e2 = st.columns(2)
            with e1:
                current_work_type = row.work_type if row.work_type in work_types else "Unassigned"
                edited_work_type = st.selectbox(
                    f"Work Type #{row.id}",
                    work_types,
                    index=work_types.index(current_work_type),
                    key=f"{reset_key}_edit_work_type_{row.id}",
                )
            with e2:
                current_subject = row.subject if row.subject in subjects else "Unassigned"
                edited_subject = st.selectbox(
                    f"Subject #{row.id}",
                    subjects,
                    index=subjects.index(current_subject),
                    key=f"{reset_key}_edit_subject_{row.id}",
                )
            edited_notes = st.text_area(
                f"Notes / description #{row.id}",
                value=str(row.notes or "") if pd.notna(row.notes) else "",
                height=90,
                key=f"{reset_key}_edit_notes_{row.id}",
            )
            st.caption(f"Date Entered: {format_date_report(row.created)}")

            if allow_active_conversion:
                a1, a2, a3, a4, a5 = st.columns(5)
            else:
                a1, a2, a3 = st.columns(3)
                a4 = a5 = None

            if a1.button("Save", key=f"{reset_key}_save_{row.id}"):
                save_item(
                    row.id,
                    edited_title,
                    ...
                    edited_work_type,
                    edited_subject,
                    row.priority_type if str(row.priority_type or "").strip() else "Task Repository",
                    edited_notes.strip(),
                    None,
                    str(row.due or ""),
                    str(row.review_date or ""),
                    str(row.review_start or ""),
                    str(row.review_end or ""),
                    is_appointment=getattr(row, "is_appointment", None),
                    appointment_date=getattr(row, "appointment_date", None),
                    is_reminder=getattr(row, "is_reminder", None),
                    reminder_date=getattr(row, "reminder_date", None),
                    critical_due_date=getattr(row, "critical_due_date", None),
                    assignee_id=getattr(row, "assignee_id", None),
                )
                invalidate_repository_caches()
                st.rerun()

            if a2.button("Complete", key=f"{reset_key}_complete_{row.id}"):
            ...
            with a3:
                with st.expander("Delete", expanded=False):
                    st.warning("This permanently deletes this repository entry.")
                    if st.button("Delete Now", key=f"{reset_key}_delete_{row.id}", type="primary"):
                        delete_item(row.id)
                        invalidate_repository_caches()
                        st.rerun()

            if allow_active_conversion:
                if a4.button("Copy to Active Tasks", key=f"{reset_key}_copy_active_{row.id}"):
                    ok, message = copy_repository_item_to_active(row.id)
                    if ok:
                        st.success(message)
                    else:
                        st.error(message)
                    st.rerun()
                if a5.button("Move to Active Tasks", key=f"{reset_key}_move_active_{row.id}"):
                    ok, message = move_repository_item_to_active(row.id)
                    if ok:
                        st.success(message)
                    else:
                        st.error(message)
...
def render_editable_item_results(df_items, prefix):
    ...
    for r in df_items.itertuples():
        with st.container(border=True):
            ...
            st.write("")
            st.caption(
                f"Work Type: {r.work_type} | Subject: {subject_text} | Priority: {priority_text} | Task Repository: {project_label}"
            ...
            project_select = st.selectbox(
                f"Assign to Task Repository #{r.id}",
    ...
    for r in df_items.itertuples():
        with st.container(border=True):
            ...
            meta = [
                ...
                f"Task Repository: {project_label}",
            ]
...
def build_condensed_items_report_pdf(report_df, report_title="Condensed Task Report", filter_summary=""):
    """Create a compact landscape report with only Item #, Priority, and Task Description."""
    buffer = BytesIO()
    doc = SimpleDocTemplate(
        buffer,
        pagesize=landscape(letter),
    ...
    story = [Paragraph(pdf_escape(report_title), title_style)]
    if filter_summary:
        story.append(Spacer(1, 0.06 * inch))
        story.append(Paragraph(pdf_escape(filter_summary), styles["Normal"]))
    story.append(Spacer(1, 0.08 * inch))
    ...
    table_data = [[
        ...
        Paragraph("<b>Task Description</b>", header_style),
    ]]
        ...
        for _, row in report_df.iterrows():
            ...
            if notes:
                description_paragraph = Paragraph(
                    f"{pdf_escape(description)}<br/>{pdf_escape(notes)}",
                    normal_style,
                )
            else:
                description_paragraph = Paragraph(pdf_escape(description), normal_style)

            table_data.append([
                Paragraph(pdf_escape(row.get("id", "")), normal_style),
                Paragraph(pdf_escape(row.get("priority_type", "") or "Task Repository"), normal_style),
                description_paragraph,
            ])

    table = Table(
        table_data,
        colWidths=[0.55 * inch, 0.95 * inch, 9.0 * inch],
        repeatRows=1,
        splitByRow=1,
    )
    table.setStyle(TableStyle([
...
if st.session_state.get("active_main_tab", "iPhone Today") not in ("Task Repository", "Concept Repository"):
...
# V30: grouped navigation. Only pages in the selected section are shown.
NAVIGATION_GROUPS = {
    "Daily Work": [
        "Active Tasks",
        "Task Repository",
...
MAIN_TABS = [page for pages in NAVIGATION_GROUPS.values() for page in pages]
...
if "active_nav_group" not in st.session_state or st.session_state.active_nav_group not in NAVIGATION_GROUPS:
    st.session_state.active_nav_group = next(
        group_name
        for group_name, group_pages in NAVIGATION_GROUPS.items()
        if st.session_state.active_main_tab in group_pages
    )
...
def _set_main_tab(tab_name):
    ...
    for group_name, group_pages in NAVIGATION_GROUPS.items():
        if tab_name in group_pages:
            st.session_state.active_nav_group = group_name
            break

# First choose the type of page, then choose only from the pages in that group.
st.markdown('<div class="navigation-section-label">Navigation Section</div>', unsafe_allow_html=True)
_group_cols = st.columns(len(NAVIGATION_GROUPS), gap="small")
for _col, _group_name in zip(_group_cols, NAVIGATION_GROUPS.keys()):
    with _col:
        _group_button_type = "primary" if _group_name == st.session_state.active_nav_group else "secondary"
        if st.button(
            _group_name,
            key=f"nav_group_{_group_name}",
            type=_group_button_type,
            use_container_width=True,
        ):
            st.session_state.active_nav_group = _group_name
            if st.session_state.active_main_tab not in NAVIGATION_GROUPS[_group_name]:
                st.session_state.active_main_tab = NAVIGATION_GROUPS[_group_name][0]
            st.rerun()

st.markdown(
    f'<div class="navigation-section-label">{st.session_state.active_nav_group}</div>',
    unsafe_allow_html=True,
)
_visible_pages = NAVIGATION_GROUPS[st.session_state.active_nav_group]
for _row_start in range(0, len(_visible_pages), 5):
    _row = _visible_pages[_row_start:_row_start + 5]
...
if current_page == "Reports / Print":
    ...
    with st.form("reports_filter_form_v27_3", clear_on_submit=False):
        ...
        with sf3:
            report_layout = st.radio(
                "Choose Report Type",
                ["Detailed Report", "Condensed Task Report"],
                index=0,
                key="report_layout_v302",
                horizontal=False,
            )
    ...
    if run_report:
        ...
        st.session_state.pop("report_pdf_condensed_v301", None)
    ...
    if st.session_state.get("report_has_run_v273", False):
        ...
        if report_df.empty:
            ...
            if st.button("Prepare PDF", key="prepare_report_pdf_v302"):
                with st.spinner("Preparing PDF..."):
                    if report_layout == "Condensed Task Report":
                        st.session_state["report_pdf_condensed_v301"] = build_condensed_items_report_pdf(
                            report_df,
                            report_title="Condensed Active Tasks Report",
                            filter_summary=filter_summary,
                        )
                        st.session_state.pop("report_pdf_v273", None)
                    else:
            ...
            if st.session_state.get("report_pdf_condensed_v301"):
                st.download_button(
                    "Download / Print Condensed Task Report PDF",
                    data=st.session_state["report_pdf_condensed_v301"],
                    file_name="condensed_active_tasks_report.pdf",
                    mime="application/pdf",
                    key="download_condensed_report_pdf_v302",
                )
            elif st.session_state.get("report_pdf_v273"):
                st.download_button(
                    "Download / Print Detailed Report PDF",
                    data=st.session_state["report_pdf_v273"],
                    file_name="task_flow_filtered_report.pdf",
                    mime="application/pdf",
                    key="download_filtered_report_pdf_v302",
                )
            ...
            for g, group in section_df.groupby("work_type"):
                ...
                for r in group.itertuples():
                    ...
                    if pd.notna(r.project_id) and r.project_id:
                        proj_df = read_sql_df("SELECT title FROM items WHERE id=?", params=(int(r.project_id),))
                        if not proj_df.empty:
                            project_note = f"Task Repository: {proj_df.iloc[0]['title']}"
                    status_bits = appointment_reminder_status(r)
                    if r.item_type in ("Project", "Ideas"):
                        status_bits.append("TASK REPOSITORY REVIEW / ADVANCEMENT")
    ...
    if weekly_df.empty:
        ...
        def render_week_bucket(title, rows, prefix):
            ...
            for idx, row in enumerate(rows):
                ...
                if row.get("item_type") in ("Project", "Ideas"):
                    status_bits.append("Repository Work")
        ...
        render_week_bucket("Repository Review Attention This Week", review_rows + project_rows, "wk_review")
...
if current_page == "Active Tasks":
    st.subheader("Active Tasks")
    df_all = load_all()
    if df_all.empty:
        st.info("No active tasks yet")
    else:
        st.markdown("### Search Active Tasks")
        ...
        all_items_row_limit = st.selectbox(
            ...
            help="Cloud mode is much faster when this page does not render every editable active-task card at once.",
        )
        ...
        st.caption(f"Showing {len(df_all)} of {total_after_filters} matching active task(s). Use filters or search to avoid loading every card.")
        ...
        for r in df_all.itertuples():
            with st.container(border=True):
                ...
                st.write("")
                st.caption(
                    f"State: {r.state if pd.notna(r.state) and str(r.state).strip() else 'Open'} | Work Type: {r.work_type} | Subject: {subject_text} | Priority: {priority_text} | Assignee: {assignee_id_to_name(getattr(r, 'assignee_id', None))} | Task Repository: {project_label}"
                    + (f" | {' | '.join(status_bits)}" if status_bits else "")
                )
                ...
                project_select = st.selectbox(f"Task Repository #{r.id}", current_proj_values, index=current_proj_values.index(project_label) if project_label in current_proj_values else 0, key=f"proj_{r.id}")
                critical_due_value = clearable_date_input(
                    f"Critical Due Date #{r.id} (MM/DD/YYYY)",
                    parse_date_safe(getattr(r, "critical_due_date", "")),
                ...
                if action2.button("Mark Complete", key=f"mark_complete_{r.id}"):
                    mark_complete(r.id)
                    st.rerun()
                if action3.button("Move to Task Repository", key=f"move_to_repository_{r.id}"):
                    move_active_task_to_repository(r.id)
                    st.rerun()
                if action4.button("Reopen", key=f"reopen_{r.id}"):
                    mark_open(r.id)
                    st.rerun()
                with action5:
                    st.button(
...
if current_page == "Task Repository":
    render_repository_page(
        repository_name="Task Repository",
        repository_item_type="Ideas",
        allow_active_conversion=True,
    )


if current_page == "Concept Repository":
    render_repository_page(
        repository_name="Concept Repository",
        repository_item_type="Concept",
        allow_active_conversion=False,
    )
...
if current_page == "Completed Tasks":
