...
# Version 26: TRUE Date Entered report sort dropdown. Date Entered is first, default, and uses a fresh key so Streamlit cannot reuse old Subject state.
...
if current_page == "Reports / Print":
    ...
    if report_df.empty:
        ...
        with sf1:
            # V26: Date Entered is intentionally included and set as the default report sort option.
            # A fresh widget key prevents Streamlit from reusing the older saved "Subject" value.
            report_sort_options = ["Date Entered", "Subject", "Item #", "Work Type", "Priority", "Critical Due Date"]
            report_sort_by = st.selectbox(
                "Sort By",
                report_sort_options,
                index=0,
                key="report_sort_by_v26_date_entered_included",
            )
        with sf2:
