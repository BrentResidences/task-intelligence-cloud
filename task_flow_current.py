...
def add_item(title, item_type="Task", work_type="Unassigned", subject="Unassigned", priority_type="As Needed", notes="", project_id=None,
    ...
    with conn() as c:
        c.execute(
            '''
            INSERT INTO items (title, state, effort, work_type, item_type, subject, priority_type, due, review_date, review_start, review_end, notes, project_id, is_appointment, appointment_date, is_reminder, reminder_date, critical_due_date, assignee_id, created, in_today)
            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, 0)
            ''',
            (title, "Open", "Medium", final_work_type, final_item_type, final_subject, final_priority_type, due, review_date, review_start, review_end, notes, project_id, int(bool(is_appointment)), appointment_date, int(bool(is_reminder)), reminder_date, critical_due_date, assignee_id, str(datetime.now()))
        )
    invalidate_caches()
...
def load_today():
    return pd.read_sql("""
        ...
        FROM items
        WHERE COALESCE(priority_type, 'As Needed') = 'Today'
...
def render_iphone_today_view():
    ...
    st.markdown('<div class="iphone-subtitle">Today items grouped by First, Second, and Last.</div>', unsafe_allow_html=True)

    df_today = load_today()
    if df_today.empty:
        st.info("No items are currently marked Today.")
        return
...
tab0, tab1, tab2, tab3, tab4, tab5, tab6, tab8, tab9, tab13, tab15, tab14, tab10, tab12 = st.tabs(["iPhone Today", "Today List", "Weekly Planning", "All Items", "Ideas", "Ideas / Concepts", "Fast Reschedule", "Subject Manager", "Work Type Manager", "Priority Manager", "Assignee Manager", "Reports / Print", "Completed Items", "Driving List"])
