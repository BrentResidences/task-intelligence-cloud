...
# Version 28.1: optional condensed task report added to Reports / Print.
...
def build_condensed_items_report_pdf(report_df, report_title="Condensed Task Report", filter_summary=""):
    """Create a landscape report containing only Item #, Priority, and Task Description."""
    buffer = BytesIO()
    doc = SimpleDocTemplate(
        buffer,
        pagesize=landscape(letter),
    ...
    normal_style = styles["BodyText"].clone("CondensedTaskBody")
    normal_style.fontSize = 7
    normal_style.leading = 9
    normal_style.spaceAfter = 0
    normal_style.spaceBefore = 0

    header_style = styles["BodyText"].clone("CondensedTaskHeader")
    header_style.fontSize = 7
    header_style.leading = 9
    ...
    story.append(Spacer(1, 0.06 * inch))
    story.append(Paragraph(
        f"Rows printed: {0 if report_df is None else len(report_df)}",
        styles["Normal"],
    ))
    ...
    if report_df is not None and not report_df.empty:
        for _, row in report_df.iterrows():
            table_data.append([
                Paragraph(pdf_escape(row.get("id", "")), normal_style),
                Paragraph(
                    pdf_escape(row.get("priority_type", "") or "Task Repository"),
                    normal_style,
                ),
                Paragraph(pdf_escape(row.get("title", "")), normal_style),
            ])
    ...
    table = Table(
        table_data,
        colWidths=[0.55 * inch, 0.95 * inch, 9.08 * inch],
        repeatRows=1,
        splitByRow=1,
    )
    table.setStyle(TableStyle([
...
if current_page == "Reports / Print":
    ...
    with st.form("reports_filter_form_v27_3", clear_on_submit=False):
        ...
        with sf2:
            report_sort_direction = st.selectbox(
                ...
                index=0,
                key="report_sort_direction_v273",
            )
        with sf3:
            report_layout = st.selectbox(
                ...
                index=0,
                key="report_layout_condensed_v281",
            )
    ...
    if run_report:
        ...
        st.session_state.pop("report_pdf_condensed_v281", None)
    ...
    if st.session_state.get("report_has_run_v273", False):
        ...
        if report_df.empty:
            ...
            if st.button("Prepare PDF", key="prepare_report_pdf_v281"):
                with st.spinner("Preparing PDF..."):
                    if report_layout == "Condensed Task Report":
                        st.session_state["report_pdf_condensed_v281"] = build_condensed_items_report_pdf(
                            report_df,
                            report_title="Condensed Task Report",
            ...
            if st.session_state.get("report_pdf_condensed_v281"):
                st.download_button(
                    "Download / Print Condensed Task Report PDF",
                    data=st.session_state["report_pdf_condensed_v281"],
                    file_name="condensed_task_report.pdf",
                    mime="application/pdf",
                    key="download_condensed_task_report_pdf_v281",
                )
...
if current_page == "Ideas":
    ...
    if filtered_projects_df.empty:
        ...
        project_row = read_sql_df("SELECT * FROM items WHERE id=?", params=(selected_project_id,)).iloc[0]
        st.markdown("### Idea")
