...
# Version 28 Phase 1: stable Neon connection; autocommit reads prevent idle transactions during page navigation.
...
def open_new_pg_connection():
    """Open a Neon/Postgres connection without leaving read transactions open.

    Autocommit is intentional for this Streamlit app. The program performs many
    short independent reads while navigating pages. Keeping autocommit disabled
    can leave the shared Neon connection idle inside a transaction between
    reruns, which can cause locks, stale connections, and process instability.
    """
    if not DATABASE_URL:
...
def _cached_pg_connection():
    """Keep the connection as a Streamlit resource, not inside session_state."""
    return open_new_pg_connection()


def get_session_pg_connection():
    """Return a live cached Neon connection and reconnect if Neon closed it."""
    raw = _cached_pg_connection()
    try:
        ...
        return _cached_pg_connection()


def reset_session_pg_connection():
    """Close and clear the cached database resource so the next call reconnects."""
    try:
        raw = _cached_pg_connection()
...
class CloudConnection:
    ...
    def __exit__(self, exc_type, exc, tb):
        # Autocommit keeps each statement independent, so page navigation never
        # leaves Neon idle inside a transaction. Reset only when the connection
        # itself has failed.
        if exc_type and isinstance(exc, (psycopg2.InterfaceError, psycopg2.OperationalError)):
            reset_session_pg_connection()
        return False
...
def load_filtered_report_items(
    work_type="All",
    priority="All",
    subject="All",
    reminder_mode="All",
    critical_due_mode="All",
    critical_due_date=None,
    created_start=None,
    created_end=None,
):
    """Load only report rows and columns that match the selected filters.

    This function intentionally is not cached. It runs only after the user presses
    Run / Refresh Report, so normal page navigation never loads the full items table.
    """
    columns = [
        "id", "title", "created", "work_type", "priority_type", "subject",
        "critical_due_date", "is_appointment", "appointment_date",
        "is_reminder", "reminder_date", "today_execution_order",
    ]
    sql = f"SELECT {', '.join(columns)} FROM items WHERE COALESCE(state, 'Open') != 'Complete' AND COALESCE(item_type, 'Task') NOT IN ('Project', 'Ideas')"
    params = []

    if work_type != "All":
        sql += " AND TRIM(COALESCE(work_type, '')) = ?"
        params.append(str(work_type).strip())
    if priority != "All":
        sql += " AND TRIM(COALESCE(priority_type, 'Task Repository')) = ?"
        params.append(str(priority).strip())
    if subject != "All":
        sql += " AND TRIM(COALESCE(subject, 'Unassigned')) = ?"
        params.append(str(subject).strip())
    if reminder_mode == "Only Reminders":
        sql += " AND COALESCE(is_reminder, 0) = 1"
    elif reminder_mode == "Exclude Reminders":
        sql += " AND COALESCE(is_reminder, 0) = 0"
    if critical_due_mode == "Specific Date" and critical_due_date is not None:
        sql += " AND LEFT(CAST(critical_due_date AS TEXT), 10) = ?"
        params.append(critical_due_date.strftime("%Y-%m-%d"))
    elif critical_due_mode == "Only Items With Critical Due Date":
        sql += " AND NULLIF(TRIM(CAST(critical_due_date AS TEXT)), '') IS NOT NULL"
    if created_start is not None:
        sql += " AND LEFT(CAST(created AS TEXT), 10) >= ?"
        params.append(created_start.strftime("%Y-%m-%d"))
    if created_end is not None:
        sql += " AND LEFT(CAST(created AS TEXT), 10) <= ?"
        params.append(created_end.strftime("%Y-%m-%d"))
...
if current_page == "Reports / Print":
    ...
    if st.session_state.get("report_has_run_v273", False):
        report_created_start = date_entered_start_date if date_entered_start_mode == "Use Start Date" else None
        report_created_end = date_entered_end_date if date_entered_end_mode == "Use End Date" else None
        with st.spinner("Loading report data..."):
            report_df = load_filtered_report_items(
                work_type=report_work_type,
                priority=report_priority,
                subject=report_subject,
                reminder_mode=report_reminder_mode,
                critical_due_mode=critical_due_mode,
                critical_due_date=critical_due_filter_date if critical_due_mode == "Specific Date" else None,
                created_start=report_created_start,
                created_end=report_created_end,
            )

        if report_df.empty:
            st.info("No open items matched the selected report filters.")
        else:
