...
def build_items_report_pdf(report_df, report_title="Items Report", filter_summary=""):
    ...
    def build_report_table(df_part):
        ...
        if df_part is not None and not df_part.empty:
            for _, row in df_part.iterrows():
                ...
                table_data.append([
                    Paragraph(pdf_escape(row.get("id", "")), normal_style),
                    Paragraph(pdf_escape(row.get("title", "")), normal_style),
                    Paragraph(pdf_escape(format_date_safe(row.get("created", "")) or str(row.get("created", ""))[:10]), normal_style),
        ...
        table = Table(
            table_data,
            colWidths=[0.45 * inch, 2.25 * inch, 0.75 * inch, 0.85 * inch, 0.68 * inch, 0.72 * inch, 1.0 * inch, 0.65 * inch, 0.78 * inch, 0.78 * inch],
            repeatRows=1,
        )
        table.setStyle(TableStyle([
            ("BACKGROUND", (0, 0), (-1, 0), colors.lightgrey),
    ...
    if report_df.empty:
        ...
        with sf1:
            report_sort_by = st.selectbox(
                "Sort By",
                ["Item #", "Date Entered", "Subject", "Work Type", "Priority", "Critical Due Date"],
                index=2,
                key="report_sort_by",
            )
        with sf2:
        ...
        display_cols = [c for c in ["id", "title", "created", "work_type", "today_execution_order", "priority_type", "subject", "critical_due_date", "is_appointment", "appointment_date", "is_reminder", "reminder_date"] if c in report_df.columns]
