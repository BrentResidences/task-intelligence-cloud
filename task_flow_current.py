...
# Version 9: faster cloud delete/today actions; removes checkbox-confirm rerun delay.
        ...
        for r in df_all.itertuples():
            with st.container(border=True):
                ...
                with action4:
                    with st.expander("Delete", expanded=False):
                        st.warning("This permanently deletes this item.")
                        if st.button("Delete Item Now", key=f"all_items_delete_{r.id}", type="primary"):
                            delete_item(r.id)
                            st.rerun()
