...
# Version 14: stabilizes Neon connection reuse to prevent intermittent psycopg2.InterfaceError after idle/reconnect.
...
with st.expander("Fast Delete by Item #", expanded=True):
    st.caption("Fastest delete path: enter the item number and delete directly from Neon. After deletion, the app stops before redrawing the full page.")
    with st.form("fast_delete_by_item_form", clear_on_submit=False):
        fd1, fd2 = st.columns([1, 2])
        with fd1:
            fast_delete_id = st.number_input("Item # to delete", min_value=1, step=1, key="fast_delete_item_id")
        with fd2:
            st.write("")
            st.write("")
            fast_delete_submitted = st.form_submit_button("Delete This Item # Now", type="primary")
        if fast_delete_submitted:
            delete_item(int(fast_delete_id))
            st.success(f"Deleted item #{int(fast_delete_id)}.")
            st.stop()
...
# V16: true wrapped tab-label navigation.
# Streamlit's built-in st.tabs stays in one long horizontal row, so use
# button-based page labels arranged in multiple rows. This prevents the menu
# from running off the right side of the page on desktop and iPhone.
MAIN_TABS = [
...
st.markdown("""
<style>
/* Wrapped label-tab navigation built from buttons. */
...
# Five labels per row gives the file-folder look without overflowing the page.
for row_start in range(0, len(MAIN_TABS), 5):
    row_tabs = MAIN_TABS[row_start:row_start + 5]
    cols = st.columns(len(row_tabs))
    for col, label in zip(cols, row_tabs):
        with col:
            active = st.session_state.active_main_tab == label
            if st.button(label, key=f"main_tab_{label.replace(' ', '_').replace('/', '_')}", type="primary" if active else "secondary", use_container_width=True):
...
if current_page == "All Items":
    ...
    if df_all.empty:
        ...
        st.caption(f"Showing {len(df_all)} of {total_after_filters} matching item(s). Use filters, search, or Fast Delete by Item # to avoid loading every card.")
